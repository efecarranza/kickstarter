...
describe('Campaign tests', () => {
    ...
    it('Ensures caller is the campaign manager', async () => {
        const manager = await campaign.methods.manager().call();
        assert.equal(accounts[0], manager);
    });

    it('Ensures contributor gets marked as approver', async () => {
    ...
    it('End to end test', async () => {
        ...
        await campaign.methods.createRequest('Buy something', web3.utils.toWei('5', 'ether'), accounts[1]).send({
            from: accounts[0],
            gas: '1000000'
        });
